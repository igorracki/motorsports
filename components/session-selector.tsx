...
import type { Session } from "@/lib/events-data";
...
interface SessionSelectorProps {
  ...
  savedPredictions: Record<string, Driver[]>;
}
  ...
  return (
    <div className="grid grid-cols-5 gap-2">
      {sessions.map((session) => {
        const isSelected = selectedSession === session.code;
        const hasResults = session.results && session.results.length > 0;
        const hasPrediction = !!savedPredictions[session.code];
        ...
        return (
          <button
            ...
            onClick={() => {
              if (isClickable) {
                onSelectSession(isSelected ? null : session.code);
              }
            }}
            className={cn(
          ...
          >
            ...
            <span className="text-xs font-bold sm:text-sm">{session.name}</span>
            <span
              className={cn(
