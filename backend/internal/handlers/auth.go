...
func NewAuthHandler(authService services.AuthService, userService services.UserService, cookieSecure bool) *AuthHandler {
	return &AuthHandler{
...
func (handler *AuthHandler) Register(context echo.Context) error {
	...
	if err := context.Validate(&request); err != nil {
		slog.WarnContext(ctx, "Validation failed for registration request", "error", err)
		return context.JSON(http.StatusBadRequest, models.ErrorResponse{
